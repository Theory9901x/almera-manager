...
export function ProgressRing({ percent, size = 34, strokeWidth = 4, showLabel = true, color: colorOverride }: {
  percent: number | null
  size?: number
  strokeWidth?: number
  showLabel?: boolean
  /** Color fijo (ej. identidad de modulo). Si se omite, usa el color de semaforo segun el porcentaje. */
  color?: string
}) {
  ...
  return (
    <span className="ds-progress-ring" style={{ width: size, height: size }}>
      ...
      {showLabel && (
        <span className="ds-progress-ring-label" style={{ color: hasData ? color : '#94A3B8', fontSize: size * 0.28 }}>
