    ...
    <div className="app-shell lg:flex">
      ...
      <aside className={`shell-sidebar fixed inset-y-0 left-0 z-40 flex w-80 flex-col overflow-hidden transition-transform lg:sticky lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="relative flex h-20 items-center gap-3 border-b border-white/10 px-6">
          <div className="grid h-11 w-11 place-items-center rounded-xl bg-[#8B1E2D] text-white shadow-lg shadow-black/30"><HeartPulse size={22} /></div>
          <div>
            <p className="text-sm font-black uppercase tracking-[.22em]">ALMERA</p>
        ...
        <div className="relative mx-4 mt-5 rounded-2xl border border-white/10 bg-white/[.04] p-4 backdrop-blur">
        ...
        <nav className="relative flex-1 space-y-1 overflow-y-auto px-4 py-5">
          <p className="px-3 pb-2 text-[10px] font-black uppercase tracking-[.22em] text-slate-500">Modulos iniciales</p>
          {session.modules.map(module => {
            ...
            return (
              <NavLink key={module.id} to={module.route} end={module.key === 'dashboard'} onClick={() => setOpen(false)} className={({ isActive }) =>
                `group flex items-center gap-3 rounded-xl px-3 py-3 text-sm transition ${isActive ? 'bg-[#B3263A] text-white font-black shadow-lg shadow-black/25' : 'text-slate-300 hover:bg-white/[.06] hover:text-white'}`
              }>
