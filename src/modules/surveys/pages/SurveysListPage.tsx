...
import {
  BarChart3, Check, Copy, ExternalLink, Link2, Loader2, Pencil, Plus, QrCode,
  RotateCcw, Send, Square, Trash2, X,
} from 'lucide-react'
import { motion } from 'framer-motion'
import {
  Badge, Button, Card, EmptyState, Field, Input, PageHeader, SearchBox, Select, Textarea,
...
function SurveysListContent() {
  ...
  async function load() {
    setLoading(true)
    try { setSurveys(await surveysService.list({ q: q || undefined, status: status || undefined, audience: audience || undefined })) }
    catch (cause) { toast.push('error', cause instanceof Error ? cause.message : 'No fue posible cargar las encuestas') }
    finally { setLoading(false) }
  }

  useEffect(() => { void load() }, [status, audience])
  useEffect(() => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <PageHeader
        ...
        identity={identity}
        actions={canCreate ? <Button identity={identity} onClick={() => setShowCreate(true)}><Plus size={16} /> Nueva encuesta</Button> : undefined}
      />
      ...
      {loading ? (
        ...
        <Card accent={identity.color}>
          <EmptyState icon={Link2} title="Aún no hay encuestas" description="Crea la primera encuesta y compártela con un enlace público estable." />
        </Card>
      ) : (
          ...
          {surveys.map(survey => (
            <motion.article key={survey.id} variants={fadeSlideUp} className="ds-card survey-list-card">
              ...
              <div className="survey-list-card-meta">
                ...
                <span>{survey.completed_count} respuesta{survey.completed_count === 1 ? '' : 's'}</span>
              </div>
