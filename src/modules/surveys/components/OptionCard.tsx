...
export function OptionCard({ label, emoji, selected, onClick, multiple, color, disabled }: {
  label: string
  emoji?: string
  selected: boolean
  onClick(): void
  ...
  return (
    ...
    >
      <span className={`survey-option-mark ${multiple ? 'is-square' : 'is-round'}`} style={selected ? { background: color, borderColor: color } : undefined}>
        {selected && (
