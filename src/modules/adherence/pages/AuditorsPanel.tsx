...
export default function AuditorsPanel({ areas }: { areas: Area[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-5">
        <p className="eyebrow">Acceso por área</p>
            ...
            <tbody>
              {auditors.map(auditor => (
                <tr key={auditor.membership_id}>
                  ...
                  <td>
                    <div className="flex flex-wrap gap-2">
                      {areas.map(area => {
                        ...
                        return (
                          <label key={area.id} className={`ui-badge ${checked ? 'tone-accent' : 'tone-info'}`} style={{ cursor: 'pointer' }}>
                  ...
                  <td><button className="row-action" onClick={() => void save(auditor.membership_id)} disabled={busy}><Save size={14} />Guardar</button></td>
                </tr>
              ))}
